...
旧记录缺少该字段时按 `.automatic` 解码。连接地址、密钥或协议偏好发生变化时会得到新的连接作用域，因此不会命中旧连接的协议证据；旧记录本身不被破坏。
...
4. 保留现有协议缓存 storage key 与端点指纹格式；旧的 Responses/Chat 成功缓存继续有效，Messages 作为向后兼容的新枚举值加入。
