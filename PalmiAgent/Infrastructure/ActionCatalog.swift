...
enum ActionCatalog {
    static let all: [ToolAction] = [
        ...
        .init(id: .searchWeb, category: .web, title: "网页搜索", effect: "按关键词搜索网页并返回候选 URL", details: "按设置中开启的默认搜索源搜索网页，只返回标题、URL、摘要等候选信息，不读取正文。需要精读时应继续调用网页浏览。", availability: .live),
        .init(id: .fetchStaticWebPage, category: .web, title: "网页浏览", effect: "按一个或多个 URL 读取网页标题与正文片段", details: "适合已知网址的单页或少量多页读取，不负责搜索。单次调用最多 10 个 URL，最多并行访问 10 个；执行层有总时间上限，到时返回已完成结果。", availability: .live),
