...
enum LLMToolDefinitionBuilder {
    ...
    private static func facadeDescription(
        ...
        case .fetch:
            lines = [
                "[网页] 读取一个或多个已知 URL 的完整可见文字，不自行筛选正文。默认使用 page_text；只有普通读取确实拿不到所需信息、并且已经排除网页本身无效或换过更合适来源后仍失败，才使用高资源消耗的 full_snapshot 网页归档。归档会保存页面及其图片、CSS、脚本、字体等素材，后续由 Agent 自行选择文件读取。"
            ]
        case .systemTime:
            lines = [
    ...
    private static func routingHint(for actionID: ToolActionID) -> String? {
        ...
        case .fetchStaticWebPage:
            return "用于读取已知 URL 的实际内容。默认且优先使用 page_text，它返回整个页面的可见文字，不自行选择或过滤正文；start/end 使用半开区间 [start,end)，end=-1 表示末尾。只有 page_text 确实无法取得所需信息，并且已经判断不是候选网页本身无效、换过更合适来源后仍失败，才允许使用 full_snapshot。full_snapshot 会消耗较多网络、时间和存储资源，把完整文字、原始及渲染页面、图片、CSS、脚本、字体等页面素材保存成独立文件夹；通常一次只归档一个最有价值的 URL，完成后由 Agent 根据 manifest.json 自行决定读取哪些文件。不要因为一次结果不理想就轻易调用 full_snapshot。用户已给出 URL 时直接调用，不要先重复搜索。"
        ...
        case .fetchStaticWebPage:
            return ToolJSONSchema.object(
                properties: [
                    ...
                    "mode": ToolJSONSchema.string(
                        description: "可选，默认 page_text。page_text 直接返回整个网页的可见文字。full_snapshot 是高资源消耗的完整网页归档，会保存页面及其图片、CSS、脚本、字体等素材；只有换过更合适来源且 page_text 仍无法取得有效信息时才能使用，不要轻易触发。",
